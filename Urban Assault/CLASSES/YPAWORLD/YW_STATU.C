...
*/
{
    ...
    if (num_cmds==0) {
        ...
        if (ywd->ActCmdID==0) {
            /*** ActCmdID ungültig, im New/Add-Mode neuestes Squad auswählen ***/
...
*/
{
    ...
    if (ywd->URBact == ywd->UVBact) {
        if (MR.flags & MAPF_RADAR) {
            ...
            /*** VhclStatus für Map und Finder ausfüllen ***/
            SR.VhclMapStatus.IsValid    = TRUE;
            ...
            /*** Map: Wenn RoboStatus gültig, diesen aktivieren ***/
        ...
        if (!(MR.flags & MAPF_RADAR)) {
            ...
            /*** RoboStatus für Map und Finder ausfüllen ***/
            SR.RoboMapStatus.IsValid    = TRUE;
...
*/
{
    ...
    if ((ywd->UserRobo == ywd->UserVehicle) || (ywd->UserSitsInRoboFlak)) {
        ...
        if (ywd->ActCmdr != -1) {
            ...
            /*** Fallback, kein ausgewähltes Squad ***/
        ...
        if (ACTION_DEAD == ywd->UVBact->MainState) {
            ...
            /*** ooops, Geschwader gibts nicht (mehr), ausgewähltes nehmen ***/
            if (ywd->ActCmdr != -1) {
                new_viewer = ywd->CmdrRemap[ywd->ActCmdr]->BactObject;
                return(new_viewer);
            } else {
            ...
            /*** Vehikel lebt noch ***/
            if (ywd->UVBact->BactClassID == BCLID_YPAGUN){
                /*** eine Flakstation -> nach ausgewähltem Squad! ***/
                if (ywd->ActCmdr != -1){
                    new_viewer = ywd->CmdrRemap[ywd->ActCmdr]->BactObject;
                    return(new_viewer);
                }else{
    ...
    /*** Ende der Liste erreicht, wieder an Anfang zurück ***/
    if (!new_viewer) {
        if (ywd->NumCmdrs > 0) {
